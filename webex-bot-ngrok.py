...
def process_message(message_obj):
    ...
    elif "?" in message_obj.text.lower():
        print(f"___ OTHER message received: repeat message '{message_obj.text}'")
        print("1")
        answer = chatgpt(message_obj.text)
        msg_result = api.messages.create(toPersonEmail=message_obj.personEmail, markdown="**Answer:** " + str(answer))
